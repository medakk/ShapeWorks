...
/*=========================================================================
  ...
  Purpose:   particle-based mean dense surface reconstruction,
                ...
                (5) construct a dense mean shape (i.e. mesh)
...
int main( int argc , char* argv[] )
{
    ...
    InputParams params;
    params.readParams(argv[1], 0); // 0 - WarpToMeanSpace
    ...
    vtkSmartPointer< vtkPoints > meanSparseShape = computeMeanSparseShape(params.localPointsFilenames, params.out_path, // to write aligned shapes for subsequent statistical analysis
    ...
    for (unsigned int shapeNo = 0; shapeNo < params.distanceTransformFilenames.size(); shapeNo++)
    {
        ...
        for (int j = 0; j < params.number_of_attributes; j++)
        {
            ...
            std::string outfilename = params.out_path + '/'+ utils::removeExtension(utils::getFilename(params.attribute_labels[shapeNo+j])) + ".warped.nrrd";
        ...
        std::string outfilename = params.out_path + '/'+ utils::removeExtension(utils::getFilename(params.distanceTransformFilenames[shapeNo])) + ".warped.nrrd";
    ...
    std::string meanDT_filename = params.out_prefix + ".meanDT.nrrd" ;
    std::string meanDTBeforeWarp_filename = params.out_prefix + ".meanDT_beforeWarp.nrrd" ;
