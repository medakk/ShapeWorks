 ...
 * system.
 *
 * A particle system consists of a set of particle locations and
 * domains in which those locations are defined.  For example, a
 * particle system may simply be a set of points in 3D Euclidean space
 * that are constrained to a specified bounding box.  A more complex
 * example is a set of 3D points constrained to a given surface.  The
 * itkPSMParticleSystem class is also designed to manage multiple sets of
 * points across multiple domains.  For example, one may create a
 * system of dozens of point sets that are each defined in different
 * spaces.  The itkPSMParticleSystem class can also maintain a set of
 * transformations associated with each domain that map that
 * particular domain into another coordinate frame, for example, a
 * common coordinate frame.
 *
 * All operations on the particle system must happen through the
 * interface of the PSMParticleSystem class.
 * 
 *
 * \ingroup PSM
...
public:
  ...
  /** Defines a transform class type.  One is associated with each
      domain. Transforms are affine for simplicity. */
  ...
  /** Register an attribute object with this particle system.  This
      action adds the attribute object as an observer for the particle
      system so that it will receive published updates.  For example,
      a gradient attribute object may need to know when particle
      positions have changed so that it can update its internal state
      appropriately (e.g. precompute a gradient that corresponds to
      the new particle position).  Internally, this method manages
      registering the appropriate itk::Command and itk::Object as an
      observer of this itk::Object (AddObserver).  Relevant state
      information is passed to objects in particle events.
  */
  void RegisterAttribute( PSMAttribute<VDimension> *);

  /** Invokes the set event on all particle positions, resetting them
      to their current value.  This method may be called to
      synchronize positional information among various observers which
  ...
  /** Add/Set/Remove a single particle position.  The actual position
      added or set will be returned.  If, for example, the domain
      imposes any constraints on this location (e.g. out-of-bounds,
      projection onto a surface), then the method returns the location
      after constraints are applied.  Both the Add and Set methods
      require a domain index.  If unspecified, the domain index is
      zero.  The Set method requires a specific particle index.  If
      the given index does not exist, an exception is thrown.  Set is
      intended to be used for moving particles. The
      SetTransformedPosition sets the position using a Point
      transformed by the m_Transform associated with the given
      domain.*/
  ...
  /** Return a position with index k from domain d.  Note the order in
      which the 2 integers must be specified!  The domain number is
      specified second and the position index within the domain is
      specified first.  Note that the domain index may be omitted if
      the particle system only contains a single domain.
      GetTransformedPosition returns the position premultiplied by the
  ...
  /** Doubles the number of particles of the system by splitting each
      particle into 2 particles.  Each new particle position is added
      to the system at a random epsilon distance on the surface from
      the original particle.  The new particles are added using the
      AddPosition method, so all appropriate callbacks are invoked. */
  ...
  /** Return the neighborhood of points with radius r around point p
      in domain k.  This is just a convenience method to avoid
      exposing the underlying Neighborhood
      objects. FindTransformedNeighborhoodPoints returns the list with
      all points transformed by the transform associated with the
      given domain.*/
  ...
  inline PointVectorType FindNeighborhoodPoints(const PointType &p,
                                                ...
                                                double r, unsigned int d = 0) const
  {  
    return m_Neighborhoods[d]->FindNeighborhoodPointsWithWeights(p,w,r); 
  }
  ...
  inline PointVectorType FindNeighborhoodPoints(unsigned int idx,
                                                ...
                                                double r, unsigned int d = 0) const
  {  
    return m_Neighborhoods[d]->FindNeighborhoodPointsWithWeights(this->GetPosition(idx,d),w, r); 
  }
  ...
  /** Add a domain to the particle system.  This method also allocates
      space for a list of positions that are contained within the
      domain, and a default neighborhood calculator.  The final,
      optional argument indicates the calling thread id.*/
  ...
  /** Return an iterator that points one position past the last
      element of the list of the domains. */
  ...
  /** Set the transform associated with domain i. This method will
      also compute and set the corresponding inverse transform if
      possible.  If the inverse transform cannot be computed, the
      exception is quietly handled by this method and an identity
      transform is set in its place.  The calling program is
      responsible for knowing when an inverse transform cannot be
      computed. */
  ...
  /** Return an iterator that points one position past the last
      element of the list of the transforms. */
  typename std::vector< TransformType >::const_iterator
...
private:
  PSMParticleSystem(const Self&); //purposely not implemented
  void operator=(const Self&); //purposely not implemented
  
  /** The 2D array of particle positions.  1st array axis is the domain number.
      These values may only be modified by the PSMParticleSystem class itself. */
