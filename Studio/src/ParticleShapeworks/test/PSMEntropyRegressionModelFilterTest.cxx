...
int itkPSMEntropyRegressionModelFilterTest(int argc, char* argv[] )
{
  ...
  if (argc < 2)
	...
	<< "PSMEntropyRegressionModelFilterTest parameter_file [output_path] [input_path]\n"
  ...
  try
    {
     ...
     for (unsigned int i = 0; i < pt_files.size(); i++)
       {
         ...
         std::vector<itk::PSMEntropyRegressionModelFilter<ImageType>::PointType> c;
         ...
         while (in)
           {
             itk::PSMEntropyRegressionModelFilter<ImageType>::PointType pt;
     ...
     if (project->HasVariables("regression_test"))
       {
         std::vector<double> expl = project->GetVariables("regression_test");
